...
!!! note "Подсветка кода"
    Отдельного лексера `.cyclo` для Pygments пока нет — блоки языка
    оформляются как ` ```text `, JSON — как ` ```json `.

!!! warning "Старая спека"
    Полный текст канона пока живёт в [spec-legacy.md](reference/spec-legacy.md).
    Главы вики переносят его по частям; при расхождении побеждает legacy,
    пока AGENTS.md не переключит канон на папку `docs/`.
