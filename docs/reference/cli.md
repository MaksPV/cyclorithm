# CLI

```text
usage:
  cyclo run FILE|- --start DATETIME --end DATETIME [--ndjson] [--format json|ics]
  cyclo next FILE|- [--from DATETIME] [--within DURATION] [-n K] [--ndjson] [--format json|ics]
  cyclo check FILE|-
  cyclo --version
  cyclo --help
```

- Команда `run` — окно `[start, end)`. При `end <= start` возвращается пустой
  список `events`, код возврата `0`.
- Команда `next` — первые `K` событий от момента `from` (включительно)
  в пределах `[from, from+within)`. Значения по умолчанию: `from` — текущий
  момент в системной зоне ОС (с суффиксом зоны), `within` — `366d`, `n` — `1`.
- Команда `check` — только проверки без окна: при успехе печатает `ok`,
  код возврата `0`.
- Источник `-` вместо файла — стандартный ввод; импорты `use` разрешаются
  от текущего каталога.
- Даты: `YYYY-MM-DDTHH:MM:SS[.mmm][Z|±HH:MM]`; коротко — `YYYY-MM-DD`
  (=T00:00:00), `YYYY-MM-DDTHH:MM` (=:00) с опциональным суффиксом `Z`/±HH:MM;
  `+DURATION` — дельта (для `--end` от `--start`, иначе от now). Поле
  `start_time` в файлах — строгая форма с опциональным офсетом. Имена зон
  (`MSK`, `Europe/...`) не поддерживаются нигде — только числовой офсет.
- Зона окна — офсет `start`/`from` (aware → с суффиксом, `Z` для `+00:00`);
  при наивном окне — зона `timezone` файла, иначе — без. `end` с другим офсетом
  — только свой инстант. Дефолтный `--from` (`next` без аргумента) — now
  в системной зоне ОС (берется текущий офсет числом; IANA/DST движок не знает,
  см. [семантику](semantics.md)). Дельта `+DURATION` в `--start` — от now
  вместе с его зоной.
- Коды возврата: `0` — успех; `1` — ошибка ввода, парсинга или валидации
  (текст в stderr, stdout пуст); `2` — некорректные аргументы (usage в stderr).
- Нечитаемый файл — `cannot read 'FILE': …`, код возврата `1`.
