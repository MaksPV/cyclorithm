...
- Даты: `YYYY-MM-DDTHH:MM:SS[.mmm][Z|±HH:MM]`; коротко — `YYYY-MM-DD`
  ...
  `start_time` в файлах — строгая форма с опциональным офсетом. Аббревиатуры
  (`MSK`) — только в `timezone` (см. синтаксис), в литералах — числовой офсет.
- Зона окна — офсет `start`/`from`: наивный → все `time` наивные, aware →
  все с суффиксом (`Z` для `+00:00`). `end` с другим офсетом — только свой
  инстант. Дефолтный `--from` (`next` без аргумента) — now в UTC (наивное).
