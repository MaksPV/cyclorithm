...
/// (коды E01–E15 — только валидация уже разобранного AST в ядре).
...
// Проверки E01–E15 — дело ядра над уже разобранным AST: парсер принимает
...
mod tests {
    ...
    fn parse_accepts_validation_fixtures() {
        ...
        // их ошибки — валидация (E01–E15), а не синтаксис.
        for src in [
